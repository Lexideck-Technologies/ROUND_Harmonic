...
class ROUNDModel(nn.Module):
    def __init__(self, hidden_size=64, input_dim=16):
        ...
        self.r = nn.Linear(hidden_size*2, 1)
    def forward(self, x, steps=12):
        ...
        for _ in range(steps):
            ph = self.c(ph, xp)
            H.append(ph)
        return self.r(torch.cat([torch.cos(ph), torch.sin(ph)], 1)), H
...
class ROUNDClockModel(nn.Module):
    def __init__(self, hidden_size=64, input_dim=1, output_classes=8):
        ...
        self.e = nn.Linear(input_dim, hidden_size)
        self.c = PhaseAccumulator(hidden_size)
        self.r = nn.Linear(hidden_size*2, output_classes)
    def forward(self, x, steps=12):
        p = self.e(x)
        xp = torch.stack([torch.cos(p), torch.sin(p)], 2)
        ph = torch.zeros(x.size(0), self.h, device=x.device)
...
class ROUNDTopologyLoss(nn.Module):
    def __init__(self, locking_strength=0.1):
        super().__init__()
        self.b = nn.BCEWithLogitsLoss()
        self.l = locking_strength
    def forward(self, p, y, h):
        tk = self.b(p, y)
        lk = torch.mean(torch.sin(h[-1])**2)
        return tk + self.l*lk, tk.item(), lk.item()
...
class ROUNDClockLoss(nn.Module):
    def __init__(self, locking_strength=0.1, states=8, terminal_only=False):
        ...
        self.l, self.s, self.t = locking_strength, states, terminal_only
    def forward(self, p, y, h):
        tk = self.c(p, y.squeeze().long())
        st = h[-1] if self.t else torch.stack(h)
        lk = torch.mean(torch.sin(self.s/2 * st)**2)
        return tk + self.l*lk, tk.item(), lk.item()
...
class HarmonicROUNDLoss(nn.Module):
    ...
    def forward(self, p, y, hist):
        ...
        for i, s in enumerate(self.h):
            tl += self.w[i] * torch.mean(torch.relu(torch.sin(s/2.0 * st)**2 - 0.032))
        tl /= len(self.h)
