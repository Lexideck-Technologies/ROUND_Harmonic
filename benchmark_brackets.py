...
def train_round(rid,X,Y,d):
    ...
    c=HarmonicROUNDLoss(locking_strength=0.03125,harmonics=[1,2],weights=[1,2],mode='binary',terminal_only=True)
    o=optim.Adam(m.parameters(),lr=C['lr'])
    ah=[]
    for e in range(C['epochs']):
        o.zero_grad();out,hist=m(X,steps=C['steps']);l,mse,lok=c(out,Y,hist);l.backward();o.step()
...
if __name__=="__main__":
    ...
    plt.tight_layout()
    plt.savefig(f'data/benchmark_brackets_{UID}.png', dpi=300)
    ...
    plt.title(f'ROUND Consistency: Brackets\nBatch {UID}')
    plt.colorbar()
    ...
    plt.tight_layout()
    plt.savefig(f'data/correlation_brackets_{UID}.png', dpi=300)
    P("Done.")
    L_FILE.close()
